    ...
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">Palliative Care Yet to Benefit</h1>
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1.5 px-3 rounded">
                <a href="?page=verify_fingerprint">Add</a>
            </button>
        </div>
        <!-- Users Table -->
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="text-left py-2 px-3">Address</th>
                        <th class="text-left py-2 px-3">Phone</th>
                        <th class="text-left py-2 px-3">NIN</th>
                        <th class="text-left py-2 px-3">Registered By</th>
                        <th class="text-left py-2 px-3">Registration Date</th>
        ...
        usersToDisplay.forEach(user => {
            const row = `
                <tr>
                    ...
                    <td class="py-2 px-3">${user.address}, ${user.local_govt}</td>
                    <td class="py-2 px-3">${user.phone_number}</td>
                    <td class="py-2 px-3">${user.nin}</td>
                    <td class="py-2 px-3">${user.registered_by}</td>
                    <td class="py-2 px-3">${user.date_created}</td>
                    <td class="py-2 px-3">${user.status}</td>
                    <td class="flex items-center py-2 px-3">
                        <a href="?page=beneficiary_information&user_id=${user.user_id}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mr-2">View</a>
                    </td>
                </tr>
            `;
    ...
    async function fetchUsers() {
        try {
            const response = await fetch('fetch_yet_to_benefit.php');
            if (!response.ok) {
